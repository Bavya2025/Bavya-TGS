...
const SearchableSelect = ({ placeholder, options, value, onChange, disabled }) => {
    ...
    const filteredOptions = (options || []).filter(opt => {
        const optName = typeof opt === 'string' ? opt : (opt.name || opt.location_name || opt.external_id || '');
        return String(optName).toLowerCase().includes(search.toLowerCase());
    });

    const handleSelect = (selectedOpt) => {
        const val = typeof selectedOpt === 'string' ? selectedOpt : (selectedOpt.name || selectedOpt.location_name || '');
        onChange(val);
        setIsOpen(false);
        setSearch('');
    };
        ...
        <div className="searchable-select-container" ref={dropdownRef}>
            ...
            >
                <span style={{ overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                    {value ? (options.find(o => (o.name || o.location_name) === value)?.name || value) : placeholder}
                </span>
                <ChevronDown size={14} style={{ color: '#94a3b8', flexShrink: 0, transition: 'transform 0.3s', transform: isOpen ? 'rotate(180deg)' : 'none' }} />
            </button>

            {isOpen && !disabled && (
                <div className="searchable-select-dropdown">
                    <div className="searchable-select-search-container">
                        <Search size={14} className="professional-input-icon" style={{ left: '0.75rem' }} />
                        <input
                            autoFocus
                            type="text"
                    ...
                    <div className="searchable-select-list no-scrollbar">
                        ...
                        {filteredOptions.length > 0 ? (
                            filteredOptions.map((opt, idx) => {
                                const optName = opt.name || opt.location_name || `opt-${idx}`;
                                return (
                                    <button
                                        key={optName}
                                        type="button"
                                    ...
                                    >
                                        <span className="flex items-center justify-between w-full">
                                            <span>{optName}</span>
                                            {(opt._detectedType || opt.location_type) && (
                                                <span className="text-[10px] opacity-50 px-2 py-0.5 bg-slate-100 rounded-md uppercase font-bold ml-2">
                                                    {opt._detectedType || opt.location_type}
                                                </span>
                                            )}
                                        </span>
                            ...
                            <div className="searchable-select-empty">
                                No {placeholder.toLowerCase().replace('all ', '')} found
                            </div>
                        )}
                    </div>
                </div>
