...
class _ProfilePageState extends State<ProfilePage> {
  ...
  Future<void> _fetchDetailedProfile() async {
    ...
    try {
      ...
      final response = await _apiService.get('/api/employees/?employee_id=$empId');
  ...
  Widget build(BuildContext context) {
    final employeeName = _profileData?['name'] ?? _profileData?['employee']?['name'] ?? _userData?['name'] ?? widget.username;
    final employeeCode = _profileData?['employee_code'] ?? _profileData?['employee']?['employee_code'] ?? _userData?['username'] ?? widget.username;
    final photo = _profileData?['photo'] ?? _profileData?['employee']?['photo'];
    final phone = _profileData?['phone'] ?? _profileData?['employee']?['phone'] ?? _userData?['phone'] ?? '';
    final email = _profileData?['email'] ?? _profileData?['employee']?['email'] ?? _userData?['email'] ?? '';

    final designation = _profileData?['role'] ?? _profileData?['position']?['name'] ?? _userData?['role'] ?? '';
    final department = _profileData?['department'] ?? _profileData?['position']?['department'] ?? '';
    final section = _profileData?['section'] ?? _profileData?['position']?['section'] ?? '';
    final List<dynamic> managers = _profileData?['reporting_to'] ?? _profileData?['position']?['reporting_to'] ?? [];
    ...
    final district = _profileData?['office']?['geo_location']?['district'] ?? '';
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        leading: Navigator.canPop(context) 
            ...
            : null,
        title: Text('My Profile', style: GoogleFonts.interTight(color: const Color(0xFF0F172A), fontWeight: FontWeight.w900)),
        centerTitle: true,
      ),
      body: _isLoading
          ...
          : RefreshIndicator(
              ...
              child: SingleChildScrollView(
                ...
                child: Column(
                  children: [
                    _buildPremiumIdentityCard(employeeName, designation, employeeCode, department, email, phone, photo),
                    ...
                    _buildInfoSection(
                      ...
                      children: [
                        ...
                        _buildInfoItem('State, Country', '$state${state.isNotEmpty ? ", " : ""}$country'),
                      ],
                    ),
  ...
  Widget _buildPremiumIdentityCard(String name, String role, String code, String dept, String email, String phone, String? photo) {
    return Container(
      ...
      child: Column(
        children: [
              ...
              Container(
                ...
                child: ClipRRect(
                  ...
                  child: photo != null && photo.isNotEmpty
                      ? Image.network(photo, fit: BoxFit.cover, errorBuilder: (_, __, ___) => const Icon(Icons.person, size: 70, color: Color(0xFF94A3B8)))
          ...
          const SizedBox(height: 24),
          Text(name, style: GoogleFonts.interTight(fontSize: 24, fontWeight: FontWeight.w900, color: const Color(0xFF0F172A))),
          const SizedBox(height: 4),
          Text(role, style: GoogleFonts.inter(fontSize: 15, color: const Color(0xFF64748B), fontWeight: FontWeight.w600)),
          const SizedBox(height: 24),
          Row(
          ...
          const Padding(padding: EdgeInsets.symmetric(horizontal: 30, vertical: 24), child: Divider(color: Color(0xFFF1F5F9))),
          Padding(
            padding: const EdgeInsets.symmetric(horizontal: 30),
            child: Column(
              children: [
                _contactTile(Icons.alternate_email_rounded, 'Email Address', email),
                const SizedBox(height: 16),
                _contactTile(Icons.phone_android_rounded, 'Mobile Phone', phone),
              ],
            ),
          ),
          const SizedBox(height: 40),
  ...
  Widget _profileBadge(IconData icon, String text) {
    return Container(
      ...
      child: Row(children: [
        ...
        Text(text, style: GoogleFonts.inter(fontSize: 12, fontWeight: FontWeight.w800, color: const Color(0xFF1E293B)))
      ]),
    );
  }
    ...
    return Row(
      children: [
        ...
        Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          Text(label.toUpperCase(), style: GoogleFonts.inter(fontSize: 9, fontWeight: FontWeight.w900, color: Colors.black26, letterSpacing: 0.5)),
          Text(value.isEmpty ? '--' : value, style: GoogleFonts.inter(fontSize: 14, fontWeight: FontWeight.w700, color: const Color(0xFF0F172A))),
        ]),
      ],
    );
  }
      ...
      child: Column(
        ...
        children: [
          Row(children: [
            ...
            Text(title, style: GoogleFonts.interTight(fontSize: 18, fontWeight: FontWeight.w900, color: const Color(0xFF0F172A))),
          ]),
          const SizedBox(height: 24),
          GridView.count(
            crossAxisCount: 2,
          ...
          if (managers != null && managers.isNotEmpty) ...[
            ...
            Text('REPORTING MANAGER(S)', style: GoogleFonts.inter(fontSize: 9, fontWeight: FontWeight.w900, color: Colors.black26, letterSpacing: 0.5)),
            const SizedBox(height: 16),
            Wrap(spacing: 8, runSpacing: 8, children: managers.map((m) => _managerChip(m)).toList()),
          ],
        ],
      ),
    );
  ...
  Widget _managerChip(dynamic m) {
    ...
    return Container(
      ...
      child: Row(mainAxisSize: MainAxisSize.min, children: [
        CircleAvatar(radius: 12, backgroundColor: const Color(0xFF7C1D1D).withOpacity(0.1), child: Text(name[0].toUpperCase(), style: const TextStyle(color: Color(0xFF7C1D1D), fontWeight: FontWeight.bold, fontSize: 10))),
        const SizedBox(width: 8),
        Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          Text(name, style: GoogleFonts.inter(fontSize: 11, fontWeight: FontWeight.w800, color: const Color(0xFF0F172A))),
          if (role.isNotEmpty) Text(role, style: GoogleFonts.inter(fontSize: 9, color: const Color(0xFF64748B), fontWeight: FontWeight.w600)),
        ]),
      ]),
    );
  }
    ...
    return Container(
      ...
      child: ElevatedButton(
        ...
        child: Row(
          mainAxisAlignment: MainAxisAlignment.center,
          children: [
            Icon(!needsRequest ? Icons.camera_front_rounded : Icons.face_retouching_natural_rounded, size: 20),
            const SizedBox(width: 8),
            Text(
              !isFaceEnrolled 
                ? 'START FACE REGISTRATION'
                : (needsRequest ? 'REQUEST FACE PHOTO UPDATE' : 'RE-ENROLL FACE DATA'),
  ...
  void _handleFaceUpdateAction(bool isResetAllowed) async {
    if (isResetAllowed) {
      ...
      if (result == true) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Face updated successfully')));
      }
    } else {
      ...
      showDialog(
        ...
        builder: (context) => AlertDialog(
          title: Text('Request Face Update', style: GoogleFonts.interTight(fontWeight: FontWeight.bold)),
          content: Column(
            mainAxisSize: MainAxisSize.min,
            children: [
              Text('Your face data is locked for security. To update it, please provide a reason for your reporting manager.',
              ...
              TextField(
                ...
                decoration: InputDecoration(
                  ...
                  focusedBorder: OutlineInputBorder(borderRadius: BorderRadius.circular(12), borderSide: const BorderSide(color: Color(0xFF7C1D1D))),
                ),
                maxLines: 3,
              ),
            ],
      ...
      child: ElevatedButton(
        ...
        child: Row(
          ...
          children: [
            ...
            const SizedBox(width: 8),
            Text('LOGOUT FROM SESSION', style: GoogleFonts.inter(fontWeight: FontWeight.w900, letterSpacing: 1, fontSize: 12)),
          ],
        ),
      ),
    );
