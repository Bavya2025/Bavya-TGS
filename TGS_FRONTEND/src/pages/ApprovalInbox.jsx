...
=======
            await api.post(`/api/bulk-activities/${batchId}/${action}/`, { remarks });
            showToast(`Batch ${action}d successfully!`, 'success');
            fetchBatches();
>>>>>>> ef1d260ab4f0ff0c66d819ad5b78dde9435b14da
        } catch (error) {
...
=======
                    {/* Bulk Travel Batch Approvals */}
                    {batches.length > 0 && (
                        <div style={{ marginBottom: '24px' }}>
                            <h3 style={{ fontSize: '1rem', fontWeight: 700, color: '#8b0000', marginBottom: '12px', display: 'flex', alignItems: 'center', gap: '8px' }}>
                                <Upload size={18} /> Bulk Travel Activity Batches Pending Approval
                            </h3>
                            {batches.map(batch => (
                                <React.Fragment key={batch.id}>
                                    <div style={{ background: '#fffbeb', border: '1px solid #fbbf24', borderRadius: '10px', padding: '16px', marginBottom: '12px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '12px' }}>
                                    ...
                                    {expandedBatch === batch.id && (
                                        <div className="premium-card animate-fade-in mb-4 overflow-x-auto p-4 bg-white border border-slate-200">
                                            ...
                                            <table className="w-full text-xs border-collapse">
                                                <thead>
                                                    <tr className="bg-slate-100 text-slate-600 uppercase">
                                                        {batch.data_json && batch.data_json.length > 0 && Object.keys(batch.data_json[0]).map(key => (
                                                            <th key={key} className="p-2 border text-left">{key.replace(/_/g, ' ')}</th>
                                                        ))}
                                                    </tr>
                                                </thead>
                                                <tbody>
                                                    {(batch.data_json || []).map((row, idx) => (
                                                                    ...
                                                                    {selectedTask.details.expenses.map((exp, index) => {
                                                                        ...
                                                                        return (
                                                                            <tr key={index} className={exp.status === 'Rejected' ? 'row-rejected' : ''}>
                                                                                ...
                                                                                <td className="text-center">
                                                                                    {exp.receipt_image ? (
                                                                                        <div className="receipt-preview-mini" onClick={() => window.open(exp.receipt_image)}>
                                                    ...
                                                    <div className="odo-reading">
                                                        ...
                                                        {selectedTask.details.odometer.start_image && (
                                                            <img
                                                                ...
                                                                alt="Start Odometer"
                                                                className="odo-img"
                                                                onClick={() => window.open(selectedTask.details.odometer.start_image, '_blank')}
                                                            />
                                                        )}
                                                    </div>
                                                    ...
                                                    <div className="odo-reading">
                                                        ...
                                                        {selectedTask.details.odometer.end_image && (
                                                            <img
                                                                ...
                                                                alt="End Odometer"
                                                                className="odo-img"
                                                                onClick={() => window.open(selectedTask.details.odometer.end_image, '_blank')}
                                                            />
                                                        )}
                                                    </div>
                                                </div>
